...
object I18n {
    ...
    fun get(key: String): String {
        // The ONE hook this fork places in the string tables. Everything it changes -- the
        // product name, and three strings that say something different here -- lives in
        // BrandStrings, so I18n.kt and all nineteen translation JSONs stay byte-identical to
        // upstream and merge for free. See BrandStrings for why that was worth doing.
        val raw = if (current != "en") {
            tables[current]?.get(key)?.takeIf { it.isNotEmpty() } ?: EN[key] ?: key
        } else {
            EN[key] ?: key
        }
        return BrandStrings.resolve(key, current, raw)
    }
