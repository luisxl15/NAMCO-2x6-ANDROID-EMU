...
private val BASE_EN: Map<String, String> = mapOf(
    ...
    "friends.explain" to "Link Discord to show what you're playing and see which friends are in System 246. Uses your existing Discord friends — System 246 keeps no account and runs no server. Only while the app is open.",
    ...
    "friends.playingNow" to "In System 246 now",
    "friends.nobody" to "None of your Discord friends are in System 246 right now.",
    "friends.notify" to "Notify me in game",
    "friends.notify.desc" to "Show a message over the game when a friend starts playing System 246.",
    ...
    "about.pcsx2.description" to "System 246 is built on the open-source PCSX2 emulator.",
    ...
    "update.checkOnLaunch.desc" to "Automatically check GitHub for a new version each time System 246 starts.",
    ...
    "app.bootLogo.desc" to "Play the System 246 intro video when the app starts.",
    ...
    "games.exit.title" to "Exit System 246?",
    ...
    "games.quickLoad.confirmBody" to "This copies the disc's files into System 246's internal storage so the game can load them directly.\n\nSpace needed: about %1\$s\nFree right now: %2\$s\n\nYou can remove them later by holding the entry it creates and choosing Remove quick-load files.",
    ...
    "hotkeys.exitToLauncher.description" to "When a game was launched from another app (e.g. ES-DE), Close Game returns to that app instead of the System 246 library.",
    ...
    "memcard.delete.body" to "Remove \"%s\" from System 246?\n\nThis deletes the card that System 246 keeps in its own memory-card folder. If you imported it from a file on your device, that original file is not touched.",
    ...
    "overlay.osdSize.description" to "Size of the on-screen display drawn over the game — FPS, stats and notifications. Does not affect the app's menus. 100% = PCSX2's default; System 246 ships 65% so it doesn't crowd a handheld screen.",
    ...
    "patches.bundled.header" to "Bundled with System 246",
    "patches.bundled.explain" to "These ship inside System 246 and are applied to this game automatically — you didn't install them. Ones with no name are always on and can't be switched off individually.",
    ...
    "patches.hostFs.folder" to "host: reads from the \"hostfs\" folder in your System 246 data folder. Put the files a game expects there — for the Biohazard Outbreak quick-load ELF, that is where its data goes.",
    ...
    "perf.fix.gamedbFixes.desc" to "Master switch for the compatibility fixes below, plus the automatic per-game fixes from System 246's game database. Leave on unless troubleshooting.",
    ...
    "renderer.importTexturePack.bootFirst" to "Boot a game first so System 246 knows the serial.",
    ...
    "setup.systemDir.intro" to "System 246 stores memory cards, save states, configs, and shader data in app-private storage by default. ",
    ...
    "setup.welcome.heading" to "Welcome to System 246 setup!",
