...
void FullscreenUI::DrawAchievementsSettingsPage(std::unique_lock<std::mutex>& settings_lock)
{
	...
	DrawToggleSetting(bsi, FSUI_ICONSTR(ICON_FA_MEDAL, "Test Unofficial Achievements"),
		// Ours keeps the ARMSX2 name; upstream's added clause is worth having, so both.
		FSUI_CSTR(
			"When enabled, ARMSX2 will list achievements from unofficial sets. These achievements are not tracked by RetroAchievements, so they unlock every time."),
